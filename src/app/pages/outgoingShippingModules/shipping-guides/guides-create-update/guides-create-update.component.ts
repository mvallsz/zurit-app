...
@Component({
  selector: "vex-guides-create-update",
  templateUrl: "./guides-create-update.component.html",
  styleUrls: ["./guides-create-update.component.scss"],
  animations: [stagger80ms, fadeInUp400ms, scaleIn400ms, fadeInRight400ms],
})
export class GuidesCreateUpdateComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<GuidesCreateUpdateComponent>,
  ...
  preSubmit() {
    if (this.minNote !== '') {
      Swal.fire({
        ...
        confirmButtonText: "Yes, create the quote!",
        ...
        if (this.quoteAirCtrl.value === true) {
          ...
          await this.guideService.createGuide(guideForm).subscribe(
            (resp: any) => {
              ...
              if (resp.ok) {
                guidesToList.push(resp.data);

                if (this.quoteSeaCtrl.value === false) {
                  this.dialogRef.close(guidesToList);
                  this.openSnackbar(resp.msg);
                }
              } else {
                this.openSnackbar(resp.msg);
