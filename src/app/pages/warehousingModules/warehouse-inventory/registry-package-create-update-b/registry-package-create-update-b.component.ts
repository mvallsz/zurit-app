...
import { TempPackageRegistryComponent } from '../registry-package-create-update/temp-package-registry/temp-package-registry.component';
...
@Component({
  selector: 'vex-registry-package-create-update-b',
  templateUrl: './registry-package-create-update-b.component.html',
  styleUrls: ['./registry-package-create-update-b.component.scss'],
...
export class RegistryPackageCreateUpdateBComponent implements OnInit {
  ...
  public defaults: WarehouseItemFull;
  ...
  ngOnInit() {
    ...
    this.tlPackageId = this.rutaActiva.snapshot.params.id;

    if (this.tlPackageId !== '0') {
      this.warehouseItemService.getWarehouseItemFullById(this.tlPackageId).subscribe((resp: ServiceResponse) => {
  ...
  public addNewPackageValidation() {
    let formOk = true;
  ...
  public async onSubmitAddPackage() {
    ...
    let formOk = this.addNewPackageValidation();
    if (!formOk) {
    ...
    if (isValidForm) {
      ...
      if (this.mode !== 'update') {
        if (this.customerPackages.length === 0) {
          if (this.addNewPackageValidation()) {
        ...
        this.warehouseItemService.createWarehouseItems(this.customerPackages).subscribe((resp: ServiceResponse) => {
          ...
          if (warehouseCreationResp.ok) {
            ...
            setTimeout(() => {
              this.openQRStickMulti(warehouseCreationResp.data);
